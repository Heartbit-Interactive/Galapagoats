...
namespace FarseerPhysics.Samples.Demos
{
    internal class AdvancedDemo2 : PhysicsGameScreen, IDemoScreen
    {
        ...
        public override void LoadContent()
        {
            ...
            bridgePath.Add(new Vector2(-5, -10));
            bridgePath.Add(new Vector2(0, 10));
            bridgePath.Add(new Vector2(5, -10));
            bridgePath.Closed = false;

            Vertices box = PolygonTools.CreateRectangle(0.2f, 0.2f);
            PolygonShape shape = new PolygonShape(box, 20);

            _bridgeBodies = PathManager.EvenlyDistributeShapesAlongPath(World, bridgePath, shape, BodyType.Dynamic, 50);
            foreach (var body in _bridgeBodies)
